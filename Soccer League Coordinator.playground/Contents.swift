...
var players: [(name: String, height: Int, experience: Bool, guardians: String)] = [
...
func countExperienced (team: [(name: String, height: Int, experience: Bool, guardians: String)]) -> Int {
    var count = 0
    for i in 0..<team.count {
        if (team[i].experience) {
            count += 1
        }
    }
    return count
...
func calcAverageHeight (team: [(name: String, height: Int, experience: Bool, guardians: String)]) -> Double {
    var sumHeight = 0
    for i in 0..<team.count {
...
// 1. We have to sort dictionaries by Experience and Height
...
func findMaxUseful(array: [(name: String, height: Int, experience: Bool, guardians: String)]) -> Int {
    if (array.isEmpty) {
        return -1
    }
    var indexMax = 0
    ...
    for i in 0..<array.count {
        if (array[i].experience) {
            useful = 50 + array[i].height
        } else {
            useful = array[i].height
        }
...
func spreadPlayers (orderOfAppend: [Int], playersArray: [(name: String, height: Int, experience: Bool, guardians: String)]) ->
    ([(name: String, height: Int, experience: Bool, guardians: String)],
    [(name: String, height: Int, experience: Bool, guardians: String)],
    [(name: String, height: Int, experience: Bool, guardians: String)]) {
    var players = playersArray
    let team : [(name: String, height: Int, experience: Bool, guardians: String)] = []
...
func printReportAboutSpread (orderOfAppend: [Int], playersArray: [(name: String, height: Int, experience: Bool, guardians: String)]) {
...
for i in 0..<namesOfTeams.count {
    var team : [(name: String, height: Int, experience: Bool, guardians: String)] = []
    ...
    for j in 0..<team.count {
        print("\(j+1). " + writeLetter(teamName: namesOfTeams[i], playerName: team[j].name, guardians: team[j].guardians, dateFirstTraining: "12/10/2016"))
    }
}
